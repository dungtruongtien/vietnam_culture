...
export async function generateStaticParams() {
  ...
  const locales = ['vi', 'en'];
  const params: { locale: string; type: string; slug: string; foodSlug: string }[] = [];
  for (const province of provinces) {
    const foodItems = await getFoodItemsByProvince(province.id);
...
type Props = {
  params: Promise<{ locale: string; type: string; slug: string; foodSlug: string }>;
};

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale, slug, foodSlug } = await params;
  const food = await getFoodItemBySlug(foodSlug);
  if (!food) return {};
  const isVi = locale === 'vi';
  const title = isVi ? food.title_vi : food.title_en;
  const description = isVi ? food.lede_vi : food.lede_en;
...
export default async function FoodItemPage({ params }: Props) {
  ...
  const [province, food] = await Promise.all([
    getProvinceBySlug(slug),
    getFoodItemBySlug(foodSlug),
  ]);

  if (!province || !food) notFound();

  const isVi = locale === 'vi';
  const provinceName = isVi ? province.name_vi : province.name_en;
  const provinceTypeSlug = isVi ? province.type : province.type_en;

  const [sources, allFoodItems] = await Promise.all([
    getFoodItemSources(food.id),
    getFoodItemsByProvince(province.id),
  ]);

  const idx = allFoodItems.findIndex((f) => f.slug === foodSlug);
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: title, item: `${siteUrl}/${locale}/${provinceTypeSlug}/${slug}/am-thuc/${foodSlug}` },
    ],
  };
